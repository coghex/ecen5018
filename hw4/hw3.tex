...
in that subgame, if that subgame is not reached by the subgame perfect equilibria for $\Gamma$, then
that subgame has no effect on the optimal history.  Recall that the subgame perfect
equilibrium takes its definition from:
\[
  O_h(s_{-i}^*|_h,s^*_i) \succsim_i|_h O_h(s_{-i}^*|_h,s_i)
\]
and that if there was a strategy that was preferable, it would have been taken in the first place.
This comes from the fact that at the point in time when you reach that subgame, your strategy has
only changed for that small subset of points behind that subgame, and the optimal strategy would
still give you the terminal history at the begining of the new subgame in $\Gamma'$.
