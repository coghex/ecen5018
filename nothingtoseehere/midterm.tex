...
\begin{enumerate}
  ...
  The important thing to note is the difficulty of remaining at an equilibrium.  Suppose that we are at $a_1b_3$.  We
  will find it difficult to move out of that space for sufficiently small $\epsilon$.  In fact, we can see that moving
  from one NE to the other would require overcoming a resistance on the order of $\epsilon^1$.
  ...
  to remain at an equilibrium, however, is essentially the same, on the order of $\epsilon^1$.  Variation \#2 will
  stay at an equilibrium a little longer however, since there is a uniform probability that a player will not do
